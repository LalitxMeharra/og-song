...
// --- CSS fix for image sizing (injected) ---
(function injectImageStyles() {
  ...
  style.textContent = `
    ...
    .grid-card { width: 100%; max-width: 160px; }
    .result-card img { width: 50px; height: 50px; flex-shrink: 0; }
    #playerCover, #mpCover { width: 100%; max-width: 300px; margin: 0 auto; }
  `;
...
async function loadHomeData() {
  try {
    ...
    const buildCards = (arr) => (arr || []).map(i => {
      ...
      return `<div class="grid-card" onclick="handleCardClick('${i.id}', '${type}', '${title.replace(/'/g, "\\'")}', '${img}')">
...
window.handleCardClick = async function (id, type, title, img) {
  ...
  if (type === 'song') {
    ...
    try {
      ...
      if (data.topSongs && data.topSongs.length > 0) {
        ...
        document.getElementById('collectionList').innerHTML =
          `<div style="text-align:center; padding: 20px;">No tracks found.</div>`;
      }
    } catch (err) {
      document.getElementById('collectionList').innerHTML =
        `<div style="text-align:center; padding: 20px; color:var(--crimson);">Error loading artist.</div>`;
    }
  } else {
    isSearchContext = false;
    ...
    try {
      ...
      document.getElementById('collectionList').innerHTML =
        `<div style="text-align:center; padding: 20px; color:var(--crimson);">Error loading collection.</div>`;
    }
  }
};
...
function setupCollectionHeader(title, img, subtitle) {
  ...
  document.getElementById('collectionList').innerHTML =
    `<div style="text-align:center; padding: 40px; font-family:'Space Mono'; color:var(--crimson);">Loading Tracks...</div>`;
  ...
  let html = songs.map((r, index) => {
    ...
    return `
      <div class="result-card" onclick="openTrack('${r.pid}')" style="display:flex; align-items:center; gap:12px; padding:8px; border:2px solid ${isPlaying ? 'var(--crimson)' : 'var(--border-dark)'}; background:${isPlaying ? '#fdf2f1' : 'var(--cream-card)'}; cursor:pointer; margin-bottom:8px;">
        ...
        ${isPlaying ? `<div style="color:var(--crimson); font-size:12px; font-weight:bold; padding-right:8px;">▶ PLAYING</div>` :
...
window.changeArtistPage = async function (direction) {
  ...
  document.getElementById('collectionList').innerHTML =
    `<div style="text-align:center; padding: 40px; font-family:'Space Mono'; color:var(--crimson);">Loading Page ${currentArtistPage + 1}...</div>`;
  ...
  try {
    ...
    if (data.topSongs && data.topSongs.length > 0) {
      ...
      document.getElementById('collectionList').innerHTML =
        `<div style="text-align:center; padding: 20px;">No more tracks on this page.</div>
         <button class="btn-page" onclick="changeArtistPage(-1)" style="padding:12px; background:var(--cream-card); border:2px dashed var(--border-dark); font-weight:bold; cursor:pointer;">← GO BACK</button>`;
    }
  } catch (err) {
    document.getElementById('collectionList').innerHTML =
      `<div style="text-align:center; padding: 20px; color:var(--crimson);">Error loading page.</div>`;
  }
};
...
async function openTrack(pid) {
  ...
  try {
    ...
    if (!data.success) {
      ...
      playNextSong();          // immediate skip, no delay
      return;
    }
    ...
    document.getElementById('btn320').href =
      `/api/download?url=${encodeURIComponent(data.links['320'])}&filename=${safeTitle}&quality=320kbps`;
    document.getElementById('btn160').href =
      `/api/download?url=${encodeURIComponent(data.links['160'])}&filename=${safeTitle}&quality=160kbps`;
    document.getElementById('btn96').href =
      `/api/download?url=${encodeURIComponent(data.links['96'])}&filename=${safeTitle}&quality=96kbps`;
    ...
    // ---- AUTO-PLAY (with fallback if blocked) ----
    const playPromise = audio.play();
    if (playPromise !== undefined) {
      playPromise.catch(() => {
        // Autoplay blocked – keep UI in paused state, user can tap play
        setPlayState(false);
...
function renderPlayerQueue() {
  ...
  if (!hasExtendedQueue) {
    html += `<div style="text-align:center; padding: 16px;">
               <button class="btn-page" onclick="extendQueue()" id="extQueueBtn" style="padding:12px; background:var(--cream-card); border:2px dashed var(--border-dark); font-family:'Space Mono'; font-weight:bold; font-size:12px; width:100%; cursor:pointer;">LOAD MORE RELATED ↓</button>
             </div>`;
  }
...
function renderArchive() {
  ...
  if (!archive.length) {
    list.innerHTML = `<div style="padding:24px;text-align:center;border:2px dashed var(--border-dark);">No playback history.</div>`;
    return;
  }
...
function renderFavorites() {
  ...
  if (!favs.length) {
    grid.innerHTML = `<div style="font-size:12px;color:var(--text-muted);padding:10px;">No favorites yet.</div>`;
    return;
  }
  grid.innerHTML = favs.map(i => `
    <div class="grid-card" onclick="isSearchContext=false; currentContextList=JSON.parse(localStorage.getItem('og_favorites')||'[]'); openTrack('${i.pid}')">
      <img src="${i.image}">
...
function renderLibrary() {
  ...
  if (!libs.length) {
    list.innerHTML = `<div style="padding:24px;text-align:center;border:2px dashed var(--border-dark);">No saved songs.</div>`;
    return;
  }
...
function toggleStorage(key, btnElem, activeIcon, inactiveIcon, addMsg, removeMsg) {
  ...
  if (items.find(s => s.pid === currentSongData.pid)) {
    ...
    btnElem.textContent = inactiveIcon;
    btnElem.classList.remove('active');
    showToast(removeMsg);
  } else {
    items.unshift(currentSongData);
    btnElem.textContent = activeIcon;
...
document.getElementById('favBtn').addEventListener('click', () => {
  toggleStorage('og_favorites', document.getElementById('favBtn'), '♥', '♡', 'Added to Favorites ♥', 'Removed from Favorites');
  renderFavorites();
});
document.getElementById('libBtn').addEventListener('click', () => {
  toggleStorage('og_library', document.getElementById('libBtn'), '✓', '+', 'Added to Library ✓', 'Removed from Library');
  renderLibrary();
});
