...
window.changeArtistPage = async function(direction) {
  ...
  try {
    ...
    const data = await res.json();
    if(data.topSongs && data.topSongs.length > 0) {
      currentContextList = data.topSongs; // Override for new page
      renderCollectionList(currentContextList, true);
    } else {
      ...
      fetch(`/api/recommend?lang=${data.language || 'hindi'}`)
        ...
        .then(radioSongs => {
          if (radioSongs && radioSongs.length > 0) {
            ...
            currentTrackIndex = 0;
            renderUpNextUI(); // Updates the UI queue
          }
...
function renderUpNextUI() {
  const upNextEl = document.getElementById('upNextBox');
  if(!upNextEl) return;
  if(currentTrackIndex >= 0 && currentTrackIndex < currentContextList.length - 1) {
    const nextSong = currentContextList[currentTrackIndex + 1];
    upNextEl.innerHTML = `<div style="font-size:10px; color:var(--crimson); font-weight:bold; letter-spacing:1px; margin-bottom:4px;">UP NEXT</div><div style="font-size:12px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${nextSong.title} - ${nextSong.artist}</div>`;
  } else {
    upNextEl.innerHTML = `<div style="font-size:10px; color:var(--text-muted);">End of Queue</div>`;
  }
}
